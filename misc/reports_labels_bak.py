...
def storage_version_batch_func():
    conn = get_db_connection()
    ids = parse_id_list(request.form.get('id_list', ''))
    if not ids: return "No Version IDs provided", 400

    with conn.cursor() as cursor:
        query = """
            SELECT ov.id, art.artist, alb.album, cat.catalogue_number, loc.storage_location,
            ...
            WHERE ov.id IN %s ORDER BY alb.album ASC
        """
        cursor.execute(query, (tuple(ids),))
        batch_data = cursor.fetchall()
    conn.close()
...
def report_top_100_func():
    ...
    with conn.cursor() as cursor:
        query = """
            ...
            JOIN lookup_artist art ON m.artist_id = art.id
            JOIN lookup_album alb ON m.album_id = alb.album_id
            GROUP BY m.id ORDER BY play_count DESC LIMIT 100
        """
        cursor.execute(query)
        report_data = cursor.fetchall()
    conn.close()
    return render_template('report_top_listened.html', report_data=report_data)
...
def report_unplayed_func():
    ...
    with conn.cursor() as cursor:
        ...
        report_data = cursor.fetchall()
    conn.close()
    return render_template('report_unplayed.html', report_data=report_data)
...
def report_by_artist_func():
    ...
    with conn.cursor() as cursor:
        ...
        if artist_id:
            cursor.execute("SELECT id, album FROM lookup_album WHERE artist_id = %s ORDER BY album ASC", (artist_id,))
            albums = cursor.fetchall()
    conn.close()
    return render_template('report_by_artist.html', artists=artists, albums=albums)
